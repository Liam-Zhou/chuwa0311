...
//                .map(List::getFirst) // only LinkedList has getFirst
//                .map(Item::getName);
        return Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty()) // Optional make sure it is not null, we need to ensure the list is not empty ---> null VS empty list
...
//                .map(items -> items.stream().map(Item::getPrice).reduce(0.0, Double::sum)) // also good
//                .orElse(0.0);
        return customer.map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        return Optional.ofNullable(customer)
                ...
                .map(items -> items.get(0))
                .map(Item::getName)
                .orElseGet(ShoppingCartUtil::getRandomAlternativeItem); // similar to orElse
    }
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .map(items -> items.get(0))
                .map(Item::getName)
                .orElseThrow(() -> new EmptyCartException("empty")); // need a supplier --> to generate or supply values without taking any input.
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        // write you code
        boolean sign =  Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .isPresent();
    ...
    public static void printItemsInCart(Customer customer) {
        ...
        Optional.ofNullable(customer)
                ...
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .ifPresent(items -> items.stream().map(Item::getName).forEach(System.out::println));

//        Optional.ofNullable(customer)
//                .map(Customer::getShoppingCart)
//                .map(ShoppingCart::getItems)
//                .filter(items -> !items.isEmpty())
//                .map(items -> items.stream().map(Item::getName).forEach(System.out::println)) // However, there's a small issue with the use of .map()
//                          in combination with forEach(), which doesn't return a value and therefore cannot be used directly within a map() operation.
    }
