...
public class ShoppingCartUtil {
    ...
    /**
     * 编写一个方法，接受一个Optional<Customer>对象作为参数。如果Optional对象非空且包含的客户有购物车，
     * 计算购物车中所有商品的总价，并返回该值。如果购物车为空或不存在，返回默认值0.0。
     * @param customer
     * @return
     */
    ...
    /**
     * 假设我们想要获取客户购物车中的第一个商品名称，如果不存在，则从一个备选商品列表中随
     * 机选择一个商品名称作为默认值。
     */
    ...
    /**
     * 假设我们想要获取客户购物车中的第一个商品名称，如果购物车为空或不存在，
     * 抛出一个自定义异常EmptyCartException。
     *
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .flatMap(items -> items.stream().findFirst())
                .map(Item::getName)
                .orElseThrow(()->new EmptyCartException("ShopingCart does not exist"));
    }

    /**
     * 假设我们想要检查客户购物车中是否有商品，并打印相应的消息。
     *
     * 编写一个方法，接受一个Customer对象作为参数，然后使用isPresent()检查购物车中是否有商品，
     * 并打印相应的消息：
     *
     * 我们使用了isPresent()方法来检查Optional对象是否非空。我们首先从Customer对象中获取购物车商品列表，
     * 然后检查其是否为空。
     ...
     *
     * 编写一个方法，接受一个Customer对象作为参数，然后使用ifPresent()
     * 处理购物车中的商品列表，并打印商品名称.
     *
