package org.chuwa.Java8Practice.excercise;
...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        return Optional.ofNullable(customer).map(c -> c.getShoppingCart()).map(sc -> sc.getItems())
                .filter(items -> !items.isEmpty()).map(items -> items.get(0)).map(item -> item.getName());
    }
    ...
    public static double getTotalPrice(Optional<Customer> customer) {
        return customer.map(c -> c.getShoppingCart()).map(sc -> sc.getItems()).filter(items -> !items.isEmpty())
                .map(items -> items.stream().mapToDouble(Item::getPrice).sum()).orElse(0.0);
    }
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        return Optional.ofNullable(customer).map(c -> c.getShoppingCart()).map(sc -> sc.getItems())
                .filter(items -> !items.isEmpty()).map(items -> items.get(0)).map(item -> item.getName()).orElseGet(() -> ShoppingCartUtil.getRandomAlternativeItem());
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        // write you code
        Optional<List<Item>> op = Optional.ofNullable(customer).map(c -> c.getShoppingCart()).map(sc -> sc.getItems());

        if (op.isPresent() && !op.get().isEmpty()) {
    ...
    public static void printItemsInCart(Customer customer) {
        // write your code
        Optional<List<Item>> op = Optional.ofNullable(customer).map(Customer::getShoppingCart).map(ShoppingCart::getItems);
        op.ifPresent(items -> {
            if (!items.isEmpty()) {
